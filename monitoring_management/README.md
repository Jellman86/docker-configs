...
The starter target uses `auth: public_v2` and `module: if_mib`, both provided by the bundled `prom/snmp-exporter` configuration. Replace `192.168.1.1` before expecting successful scrapes.
...
Use SNMPv3 wherever possible. If you need custom auth profiles or vendor MIB modules, generate a custom `snmp.yml` with the `prometheus/snmp_exporter` generator and mount it into the `snmp-exporter` service.
